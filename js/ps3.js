(function() {
    ...
    async function cargarProductos() {
        try {
            console.log('1. Intentando cargar JSON PS3...');
            
            const respuesta = await fetch('json/play3.json');
            ...
            console.log('3. Productos PS3 cargados:', productos);
            console.log('4. Cantidad de productos:', productos.length);
            
            mostrarProductos();
            
        } catch (error) {
            console.error('❌ ERROR PS3:', error);
            document.getElementById('productosGridPS3').innerHTML = `
    ...
    function mostrarProductos() {
        const grid = document.getElementById('productosGridPS3');
        
        if (!grid) {
            console.error('No encontré el elemento productosGridPS3');
            return;
        }
    ...
    document.addEventListener('DOMContentLoaded', () => {
        console.log('Página cargada, iniciando PS3...');
        cargarProductos();
    });
})();